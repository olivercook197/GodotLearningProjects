	...
	for global_stat in lifetime_stats:
		if global_stat == MOST_XP_GAINED:
			...
			for level_up in current_run_stats[global_stat]:
				if int(level_up) not in lifetime_stats[global_stat] and float(level_up) not in lifetime_stats[global_stat]:
					lifetime_stats[global_stat].append(int(level_up))
		else:
