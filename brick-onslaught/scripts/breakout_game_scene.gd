...
func start_scene():
	...
	if GlobalVariables.ball_speed < 400 + 10 * GlobalVariables.stage:
		GlobalVariables.ball_speed = 400 + 10 * GlobalVariables.stage
...
func _on_world_border_body_entered(body: Node2D) -> void:
	print("Entered world border: ", body)
	if body is not Laser:
		if !body.slow_ball:
