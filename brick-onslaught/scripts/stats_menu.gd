...
@onready var delete_data_button: Panel = $MarginContainer/VBoxContainer/HBoxContainer/DeleteDataButton
...
func display_all_time_stats():
	...
	title.custom_minimum_size.x = 1255
	delete_data_button.custom_minimum_size = Vector2(60, 60)
...
func _on_delete_button_hover_animator_confirmed(button) -> void:
	control.visible = true


func _on_yes_delete_button_pressed() -> void:
	MetaStats.delete_all_stats()
	for stat in MetaStats.lifetime_stats:
		if stat != MetaStats.UNIQUE_LEVEL_UPS_TAKEN:
			MetaStats.lifetime_stats[stat] = 0
		else:
			MetaStats.lifetime_stats[stat] = []
	
	SaveLoad.delete_score()
	GlobalVariables.high_score = 0
...
func _on_no_delete_button_2_pressed() -> void:
